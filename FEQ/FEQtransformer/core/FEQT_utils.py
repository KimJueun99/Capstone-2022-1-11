...
from keras.layers import add, Activation, LSTM, Conv1D,GRU
from keras.layers import SeparableConv1D, MaxPooling1D, UpSampling1D, Cropping1D, SpatialDropout1D, Bidirectional, BatchNormalization
...
######################################
### Change to use bottleneck structure
...
def _encoder(filter_number, filter_size, depth, drop_rate, ker_regul, bias_regul, activation, padding, inpC):
    ...
    for dp in range(depth):
        if dp>=1 and filter_number[dp-1] == filter_number[dp] and filter_number[dp] == filter_number[dp+1]:
            ...
            if dp>=2 and filter_number[dp-2] == filter_number[dp-1] and filter_number[dp-1] == filter_number[dp]:
                e = shared_layers(e)
            else:
...
class cred2():
    ...
    """
    #####################
    # Change default filters, kernel size, number of blocks
    def __init__(self,
                 nb_filters=[8, 16, 16, 16, 32],
    ...
    def __call__(self, inp):
        ...
        for cb in range(self.cnn_blocks):
            if cb > 2:
                x = _block_CNN_1(self.nb_filters[4], 2, self.drop_rate, self.activationf, self.padding, x)
            else:
                x = _block_CNN_1(self.nb_filters[4], 3, self.drop_rate, self.activationf, self.padding, x)
        ...
        PGRU = GRU(self.nb_filters[1], return_sequences=True, dropout=self.drop_rate, recurrent_dropout=self.drop_rate)(encoded)
        ...
        SGRU = GRU(self.nb_filters[1], return_sequences=True, dropout=self.drop_rate, recurrent_dropout=self.drop_rate)(encoded)
